...
def is_component_valid(object, component_name):
    if "components_meta" in object or hasattr(object, "components_meta"):
        ...
        # Prnt the component metadata for debugging
        component_meta = next(
            filter(
                lambda component: component.name == component_name,
                target_components_metadata,
            ),
            None,
        )
...
def remove_unwanted_custom_properties(object):
    ...
    component_names = list(
        object.keys()
    )  # to avoid 'IDPropertyGroup changed size during iteration' issues
...
def copy_animation_data(source, target):
    if source.animation_data:
        ...
        with bpy.context.temp_override(
            active_object=source, selected_editable_objects=[target]
        ):
...
def duplicate_object(
    object, parent, combine_mode, destination_collection, blueprints_data, nester=""
):
    copy = None
    internal_blueprint_names = [
        blueprint.name for blueprint in blueprints_data.internal_blueprints
    ]
    # print("COMBINE MODE", combine_mode)
    if object.instance_type == "COLLECTION" and (
        combine_mode == "Split"
        or (
            combine_mode == "EmbedExternal"
            and (object.instance_collection.name in internal_blueprint_names)
        )
    ):
        ...
        for component_name, component_value in object.items():
            if (
                component_name not in custom_properties_to_filter_out
                and is_component_valid(object, component_name)
...
def copy_hollowed_collection_into(
    source_collection,
    destination_collection,
    parent_empty=None,
    filter=None,
    ...
    collection_instances_combine_mode = getattr(
        addon_prefs, "collection_instances_combine_mode"
    )

    for object in source_collection.objects:
        if object.name.endswith(
            "____bak"
        ):  # some objects could already have been handled, ignore them
            continue
        ...
        combine_mode = (
            ...
            else collection_instances_combine_mode
        )
        parent = parent_empty
        duplicate_object(
            object, parent, combine_mode, destination_collection, blueprints_data
        )
    ...
    for collection in source_collection.children:
        ...
        collection_placeholder = make_empty(
            original_name, [0, 0, 0], [0, 0, 0], [1, 1, 1], destination_collection
        )
...
def get_scenes(addon_prefs):
    level_scene_names = getattr(
        addon_prefs, "main_scene_names", []
    )  # list(map(lambda scene: scene.name, getattr(addon_prefs,"main_scenes")))
    library_scene_names = getattr(
        addon_prefs, "library_scene_names", []
    )  # list(map(lambda scene: scene.name, getattr(addon_prefs,"library_scenes")))

    level_scene_names = list(
        filter(lambda name: name in bpy.data.scenes, level_scene_names)
    )
    library_scene_names = list(
        filter(lambda name: name in bpy.data.scenes, library_scene_names)
    )
