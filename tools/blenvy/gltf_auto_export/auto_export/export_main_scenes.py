...
def export_main_scene(scene, blend_file_path, addon_prefs, blueprints_data):
    ...
    assets_path_full = getattr(addon_prefs,"export_assets_path_full")
    levels_path_full = getattr(addon_prefs,"export_levels_path_full")
    ...
    if export_blueprints : 
        ...
        if export_separate_dynamic_and_static_objects:
            ...
            generate_and_export(
                ...
                temp_scene_filler=lambda temp_collection: copy_hollowed_collection_into(
                    scene.collection, temp_collection, blueprints_data=blueprints_data, filter=is_object_static, addon_prefs=addon_prefs
                ),
                temp_scene_cleaner=lambda temp_scene, params: clear_hollow_scene(
                    original_root_collection=scene.collection, temp_scene=temp_scene, **params
                )
            )

            # Export dynamic objects
            gltf_output_path_dynamic = os.path.join(export_levels_path_full, scene.name + "_dynamic")
            generate_and_export(
                addon_prefs,
                export_settings=export_settings,
                gltf_output_path=gltf_output_path_dynamic,
                temp_scene_name=TEMPSCENE_PREFIX,
                temp_scene_filler=lambda temp_collection: copy_hollowed_collection_into(
                    scene.collection, temp_collection, blueprints_data=blueprints_data, filter=is_object_dynamic, addon_prefs=addon_prefs
                ),
                temp_scene_cleaner=lambda temp_scene, params: clear_hollow_scene(
                    original_root_collection=scene.collection, temp_scene=temp_scene, **params
                )
            ...
            generate_and_export(
                ...
                temp_scene_name=TEMPSCENE_PREFIX,
                temp_scene_filler=lambda temp_collection: copy_hollowed_collection_into(
                    scene.collection, temp_collection, blueprints_data=blueprints_data, addon_prefs=addon_prefs
                ),
                temp_scene_cleaner=lambda temp_scene, params: clear_hollow_scene(
                    original_root_collection=scene.collection, temp_scene=temp_scene, **params
                )
